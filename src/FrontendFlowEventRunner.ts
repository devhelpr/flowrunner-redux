...
FlowEventRunner.useFlowNodeOverrideAttachHook((
    node : any, 
    task : any, 
    eventEmitter : any, 
    nodeEvent  :any) => {
    ...
    return function(middlewareAPI : any){
        return function(next : any){
            return function(action : any){
                ...
                subjectStoreChange.next({nextState:nextState,
                    prevState:prevState});
                return result;
            }
        }
...
let start : any = (flowPackage : any) => FlowEventRunner.start(flowPackage, services, true).then((data) => {
